...
class ConditioningMethod(ABC):
    def __init__(self, operator, noiser, **kwargs):
        ...
        self.alpha_df = pd.DataFrame(columns=['Time', 'StepMin', 'StepMax', 'StepMean'])
    ...
    def grad_and_value(self, x_prev, x_0_hat, measurement, t, use_secondtweedie, score, spatial_weight, **kwargs):
            ...
            if (use_secondtweedie==True) and (t>0):
                ...
                def trace_estimate(x_prev, score, eps=None, **kwargs):
                    ...
                    #s  = score #score_fn(z)           # ∇ log p(z)
                        s_ = kwargs['func'](kwargs['model'], (x_prev + eps), t, kwargs['clip_denoised'], kwargs['denoised_fn'], kwargs['cond_fn'], kwargs['model_kwargs'])['score'] # ∇ log p(z + ε)
                    ...
                    self.alpha_df = self.alpha_df.append({'Time': t.cpu().numpy(), 'StepMin': alpha_map.detach().cpu().numpy().min(), 'StepMax': alpha_map.detach().cpu().numpy().max(), 'StepMean': alpha_map.detach().cpu().numpy().mean()}, ignore_index=True)
                    self.alpha_df.to_csv('alpha_map_dynamicdps_0.9_skip.csv')
...
class PosteriorSampling(ConditioningMethod):
    def __init__(self, operator, noiser, **kwargs):
        ...
        self.c1 = 1e-4      # Sufficient decrease parameter
        self.c2 = 0.1       # Curvature parameter
        ...
        self.csv_file = "./line_search_stepsize_dynamicdps_fixed_0.1_c1e4_noearly.csv"
    ...
    def line_search(self, x_prev, x_t, x_0_hat, measurement, norm_grad, norm, t, use_secondtweedie, score, spatial_weight, **kwargs):
        ...
        alpha_min = 0.1 #1e-8  # Minimum step size
        ...
        for i in range(self.max_line_search):
            ...
            x_t_new = (x_t - self.alpha * norm_grad)#.clone().requires_grad_(True)
            ...
            norm_grad_new, norm_new = self.grad_and_value(x_prev=x_t_new, x_0_hat=x_0_hat_new, measurement=measurement, t=t, use_secondtweedie=use_secondtweedie, score=score, spatial_weight=spatial_weight, **kwargs)
    ...
    # Compute initial gradient and norm
        ...
        norm_grad, norm = self.grad_and_value(x_prev=x_prev, x_0_hat=x_0_hat, measurement=measurement, t=t, use_secondtweedie=self.use_secondtweedie, score=score, spatial_weight=True, **kwargs)
        ...
        self.best_ls = 1000
        #save dataframe
        self.loss_df.to_csv('measurement_loss_timestep_dynamicdps_c1e4_noearly.csv')
