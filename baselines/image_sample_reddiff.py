# image_sample.py
# --- only diffs are annotated with [reddiff] ---
import argparse
import os
...
def main():
    ...
    with open('/cluster/project0/IQT_Nigeria/skim/diffusion_inverse/baselines/configs.yaml') as file:
    ...
    save_path = '/cluster/project0/IQT_Nigeria/skim/reddiff_vanilla_lr000005_noise2_linear_sgd'
    ...
    # [reddiff] pull reddiff hyperparams (optional; defaults set inside sampler)
    reddiff_cfg = configs.get('reddiff', {})  # e.g., {'num_steps':50,'eta':1e-4,'likelihood_weight':1.0,'sigma_min':0.002,'sigma_max':80,'rho':7.0,'noise_scale':1.0}
    ...
    for i, (ref_img, data_dict) in tqdm.tqdm(enumerate(data)):
        ...
        # [reddiff] choose sampler: reddiff (EDM/Langevin) vs your original DDPM/DDIM
        use_reddiff = configs['conditioning']['params'].get('reddiff', False)
        print("Using reddiff:", use_reddiff)

        start = time.time()
        if use_reddiff:
            # [reddiff] new entry point: Langevin posterior sampling with EDM schedule
            sample = diffusion.reddiff_sample_loop(
                model,
                ...
                grad_weight=reddiff_cfg.get('eps_weight', 2.0),   # gradient weight for measurement consistency
                data_weight=reddiff_cfg.get('data_weight', 1.0),           # weight for original score (prior)
