...
def main():
    ...
    with open('/cluster/project0/IQT_Nigeria/skim/diffusion_inverse/baselines/configs_diffpir.yaml') as file:
    ...
    save_path = '/cluster/project0/IQT_Nigeria/skim/diffpir_linear_alpha2_diffpir_ddim'
    ...
    for i, (ref_img, data_dict) in tqdm.tqdm(enumerate(data)):
        ...
        if configs['skip_timestep']:
            skip_x0 = y_n #torch.tensor(data).unsqueeze(0).unsqueeze(0).to(torch.float32).to(device)
        ...
        if data_dict is not None:
            ...
            for j in range(args.batch_size):
                ...
                #np.save(f'{save_path}/{data_dict["file_id"][j]}/lr_clone{data_dict["slice_idx"][j]}_axial.npy', y_n_clone[j].cpu().numpy())
